<template>
    <div class="po-fixed fixedBg">
        <div class="appleyRes">
            <div class="appleyRes_title po-relative">
                <p>信保报告申请</p><span class="po-absolute" @click="hideshowPes"></span>
            </div>
            <div class="appleyRes_content">
                ...
                <div class="apply_info">
                    ...
                    <table class="apply_table">
                        ...
                        <tr>
                            ...
                            <td style="width: 110px"><input disabled type="text"
                                    v-model="report.reportCorpCountryCode" /></td>
                            <td style="width: 150px"><input disabled type="text" v-model="report.reportCorpChnName" />
                            </td>
                            <td style="width: 150px"><input disabled type="text" v-model="report.reportCorpEngName" />
                            </td>
                            <td style="width: 220px"><input disabled type="text" v-model="report.reportCorpaddress" />
                            </td>
                            <td style="width: 180px"><input disabled type="text" v-model="report.creditno" /></td>
                            <td style="width: 60px">
                                <select v-model="report.istranslation">
                                    <option v-for="(item,index) in istranslation" :key="index" v-bind:value="item.id"
                                        v-text="item.name"></option>
                                </select>
                            </td>
                            <td>
                                <div class="applyBtn" @click="applyAdd">点击申请</div>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="apply_info">
                    ...
                    <table class="apply_table">
                        ...
                        <tr>
                            <td style="width: 200px"><input disabled type="text" v-model="noReport.reportbuyerNo" />
                            </td>
                            ...
                            <td style="width: 60px">
                                <select v-model="noReport.istranslation">
                                    <option v-for="(item,index) in noIstranslation" :key="index" v-bind:value="item.id"
                                        v-text="item.name"></option>
                                </select>
                            </td>
                            <td>
                                <div class="applyBtn" @click="noApplyAdd">点击申请</div>
                            </td>
                        </tr>
                    </table>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /*无信保申请*/
            noApplyAdd() {
                if (this.noReport.reportCorpCountryCode === '') {
                    this.$message.warning('请输入待调查企业国别');
                    return;
                } else if (this.noReport.reportCorpChnName === '' && this.noReport.reportCorpEngName === '') {
                    this.$message.warning('请输入待调查企业中文名称或英文名称');
                    return;
                } else if (this.noReport.reportCorpaddress === '') {
                    this.$message.warning('请输入待调查企业地址');
                    return;
                } else if (this.noReport.creditno === '') {
